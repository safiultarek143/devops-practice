...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            <a href="{{ route('post-categories.create') }}" class="btn btn-info fa-pull-right"><i class="fa fa-plus"></i> Add new</a>
        </div>
        <!-- /.card-header -->
        <div class="card-body">
                ...
                @if(!empty($post_categories))
                    @foreach($post_categories as $key => $post_category)
                        <tr>
                            ...
                            <td>
                                <a class="btn btn-sm btn-info" href="{{ route('post-categories.edit',$post_category->id) }}"><i class="fa fa-edit"></i> Edit</a>
                                @include('includes.master._confirm_delete',[
                                    'id' => $post_category->id,
                                    'url' => route('post-categories.destroy',$post_category->id),
                                    'message' => 'Are you sure want to delete this Post Category?',
                                ])
                            </td>
                        </tr>
                    @endforeach
