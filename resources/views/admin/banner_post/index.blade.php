...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            <a href="{{ route('posts.create') }}" class="btn btn-info fa-pull-right"><i class="fa fa-plus"></i> Add new</a>
        </div>
        <!-- /.card-header -->
        <div class="card-body">
                ...
                @if(!empty($posts))
                    @foreach($posts as $key => $post)
                        <tr>
                            ...
                            <td>
                                <img src="{{ asset($post->image) }}" alt="No image" width="40px">
                            </td>
                            <td>{{ $post->status== 1? 'Published': 'Unpublished' }}</td>
                            <td>
                                <a class="btn btn-sm btn-info" href="{{ route('posts.edit',$post->id) }}"><i class="fa fa-edit"></i> Edit</a>
                                <a class="btn btn-sm btn-info" href="{{ route('posts.show',$post->id) }}"><i class="fa fa-edit"></i> Show</a>
